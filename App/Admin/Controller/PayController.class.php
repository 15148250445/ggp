...
// 		$admin = $this->admin;
// 		$arr=explode(',',$admin['currency_id']);
// 		$map['currency_id'] = array('in',$arr);
// 		//读取币种表
// 		$curr=M("Currency")->where($map)->select();
...
// 		}else{
// 			$data[C("DB_PREFIX")."currency.currency_id"]=array('in',$arr);
// 		}
// 		if(!empty($email)){
...
// 		//$where['yang_tibi.status'] = array('in','2,3,7');
// 		$where[C("DB_PREFIX").'tibi.currency_id'] = array('neq',$currency['currency_id']);
// 		$where['ti_id'] = '';
...
// 		->where($data)->where('( ti_id = \'\' OR '.C("DB_PREFIX").'tibi.type= 1 ) AND '.C("DB_PREFIX").'tibi.currency_id <> '.$currency['currency_id'])
// 		->count();// 查询满足要求的总记录数
// 		$Page  = new \Think\Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数(25)
...
// 						->join("left join ".C("DB_PREFIX")."currency on ".C("DB_PREFIX")."currency.currency_id = ".C("DB_PREFIX")."tibi.currency_id")
// 						->where($data)
// 						->where('(ti_id = \'\' OR '.C("DB_PREFIX").'tibi.type=1 ) AND '.C("DB_PREFIX").'tibi.currency_id <> '.$currency['currency_id'])
// 						->limit($Page->firstRow.','.$Page->listRows)
