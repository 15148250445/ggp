...
class CurrencyController extends AdminController {
	...
	public function tibi_index(){
		...
		$admin = $this->admin;
		$list=explode(',',$admin['currency_id']);
		$map['currency_id'] = array('in',$list);
		//读取币种表
		...
		if(!empty($cuid)){
			...
			$where[C("DB_PREFIX")."tibi.currency_id"]=array('in',$list);
		}
		if(!empty($email)){
		...
		//给分页传参数
		setPageParameter($Page, array('cuid'=>I("cuid"),'email'=>$email));
	...
	public function index(){
		...
		$list=$this->currency->where($map)->select();
