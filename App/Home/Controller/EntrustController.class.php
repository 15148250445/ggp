...
class EntrustController extends TradeFatherController {
	...
	public function manage(){
		...
		$postparam = array('currency'=>$currencytype,'status'=>$status);
		...
		$list = M('Orders')->Field('orders_id,member_id,currency_id,currency_trade_id,price,num,trade_num,fee,type,add_time,trade_time,status')->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();
		
		$this->assign('page',$show);// 赋值分页输出
		$this->assign('post',$postparam);// 赋值分页输出
