...
class IndexController extends CommonController {
	...
	public function index(){
		...
		if(S('gfgg_index')){
			...
			$gfgg = $art_model->where("position_id = 1")->limit(5)->order('add_time desc')->select();
			S('gfgg_index',$gfgg);
		}
		...
		if(S('scdt_index')){
			...
			$scdt = $art_model->where("position_id = 2")->limit(5)->order('add_time desc')->select();
			S('scdt_index',$scdt);
		}
		...
		if(S('mtbd_index')){
			...
			$mtbd = $art_model->where("position_id = 3")->limit(5)->order('add_time desc')->select();
			S('mtbd_index',$scdt);
		}
