...
class CommonController extends Controller {
   ...
   protected $member;
    ...
    public function _initialize(){
		...
		//分红奖励控制参数
		/*if(S('dividendConfig')){
			$this->dividendConfig = S('dividendConfig');
		}else{
			...
			S('dividendConfig',$dividendConfig);
		}
       
        
        

      //  $this->assign('dividendConfig',$this->dividendConfig);*/
		...
		if(S('info_help')){
			$info_help = S('info_help');
		}else{
			$info_help = M('Article_category')->Field('id,name')->where('parent_id = 6')->limit(4)->select();
			S('info_help',$info_help);
		}
		//团队信息显示
		
		if(S('info_team')){
			$info_team = S('info_team');
		}else{
			$info_team = M('Article')->Field('article_id,title')->where('position_id = 7')->limit(4)->select();
			S('info_team',$info_team);
		}

		$this->assign('help',$info_help);
		$this->assign('team',$info_team);
        if(!empty($_SESSION['USER_KEY_ID'])){
            $member=M("Member")->field('member_id,openid,email,pwd,pid,pwdtrade,nick,name,cardtype,idcard,phone,ip,reg_time,login_ip,login_time,rmb,forzen_rmb,head,profile,province,city,job,is_lock,status,dividend_num,threepwd,(rmb+forzen_rmb) as count')->where("member_id='{$_SESSION['USER_KEY_ID']}'")->find();
            $this->member=$member;
            $this->assign("member",$member);
        }
		if(!empty($_SESSION['USER_KEY_ID']) && $_SERVER['REQUEST_URI'] != U('Login/loginOut')&&$this->member['status'] == 2){		
				$this->redirect("Login/loginOut");
		}
		
       // $this->trade=M('Orders');  不知道做了什么用
       
        
		//币种信息
        $this->currency = $this->currency();
// 		dump($this->currency);die;
        $this->assign('currency', $this->currency);
        $this->assign('currency_back', $this->currency);
        

		$this->assign("currency_header",$this->currency);
     ...
     *个人资金优化  只有上面调用
     */
    protected function getuserallmoney($currency_id){
	...
	protected function  User_status(){
		...
		$list = M('Member')->Field('status')->where(array('member_id'=>session('USER_KEY_ID')))->find();
		if($list['status']==0){
			session('STATUS',0);
    ...
    protected function getOrdersByStatus($status,$num,$currency_id){
     ...
     protected  function getUserMoney($currency_id,$field){
         ...
         if (empty($currency_id)){
             ...
             $currency_user= M('Currency_user')->Field($field)->where("member_id={$this->member['member_id']} and currency_id=$currency_id")->find();
         }
...
//          $where['currency_id']=$currency_id;
     ...
     protected function getOrdersDataList(){
     	...
     	foreach ( $currency_or as $k => $v ) {
     		...
     		$currency_or [$k] ['new_price'] = $list ['new_price'] ? $list ['new_price'] : 0;
     		$currency_or [$k] ['currency_all_money'] = number_format ( floatval ( $v ['currency_all_num'] ) * $list ['new_price'], 2 );
     		$currency_or [$k] ['all_num'] = M ( "Trade_" . $v ['currency_id'] )->sum ( "num" );
     			
     		// 开盘价
     		$open = $this->getKlineOpen($v['currency_id']);
     		$currency_or [$k] ['kai_price'] = $open;
     		unset ( $kai );
     		// 昨日收盘
     		...
     		// 最高价格
     		$zuigao = M ( "Trade_" . $v ['currency_id'] )->field ( "price" )->order ( "price desc" )->limit ( 1 )->find ();
     		$currency_or [$k] ['zuigao'] = $zuigao ['price'];
     		// 最低价格
     		$zuidi = M ( "Trade_" . $v ['currency_id'] )->field ( "price" )->order ( "price asc" )->limit ( 1 )->find ();
