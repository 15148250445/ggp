 ...
 * @param String $email  收件人用户名，收件人姓名
...
//    $mail->Password='WWW15988999998com';//邮局密码
    ...
    $mail->AddAddress($email);// 收件人用户名，收件人姓名
 ...
 * @return mixed 错误信息
 */
/* function sandPhone_old($phone,$name,$user,$pass){
    $statusStr = array(
        "0" => "短信发送成功",
        "-1" => "参数不全",
 ...
 * @param $phone 电话号码
 * @param $user  短息接口用户名
 * @param $get_content  短信用途 	例：【火币网】您的验证码为12345,2016-8-3 14:24:8申请用于'.$get_content.'，30分钟内有效，请勿告诉他人
 * @param $code_name  session的后缀 不同验证码后缀不要相同与验证有关
 * @param $pass  短信接口密码
 * @return mixed 错误信息
 */
function sandPhone($phone,$name,$user,$pass,$content=''){
	if(empty($phone)){
		return '手机号为空';
	}
    ...
    session(array('name'=>'code','expire'=>600));
    session('code',$code);  //设置session
	session('phone_code',$phone);
    session('time',time());
	if(empty($content)){
		 $content="您的验证码是".$code."，10分钟有效，如非本人操作，请联系客服 400-9665-100。";//要发送的短信内容
	}
   

	$url="http://service.winic.org:8009/sys_port/gateway/index.asp";
	$data = "id=%s&pwd=%s&to=%s&content=%s&otime=";
	$userName = iconv("UTF-8","GB2312",$user);
	$PassWord = $pass;
	$Content = iconv("UTF-8","GB2312",$content);
	$rdata = sprintf($data, $userName, $PassWord,$phone, $Content);
