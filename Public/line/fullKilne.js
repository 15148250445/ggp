...
var GLOBAL_VAR = {
    ...
    button_down       :  false,
    url				  :  php_url,
   // url				  : "https://www.okcoin.cn/api/v1/kline.do?symbol=btc_cny&type=1min&size=1000"
};
...
Chart._future_btc_market_num = new Array('0', '11', '12', '13', '34');
Chart._future_ltc_market_num = new Array('1', '14', '15', '16', '35');
Chart.strMarket = {
    'zh-cn': {'01':'当周 '+$('#xnbname').val()+' ','02':'次周 '+$('#xnbname').val()+' ','03':'全月 '+$('#xnbname').val()+' ','04':'季度 '+$('#xnbname').val()+' ',
        '11':'当周 LTC ','12':'次周 LTC ','13':'全月 LTC ','14':'季度 LTC '},
    'en-us': {'01':''+$('#xnbname').val()+' This Week ','02':''+$('#xnbname').val()+' Next Week ','03':''+$('#xnbname').val()+' Month ','04':''+$('#xnbname').val()+' Quarter ',
        '11':'LTC This Week ','12':'LTC Next Week ','13':'LTC Month ','14':'LTC Quarter '},
    'zh-tw': {'01':'當周 '+$('#xnbname').val()+' ','02':'次周 '+$('#xnbname').val()+' ','03':'全月 '+$('#xnbname').val()+' ','04':'季度 '+$('#xnbname').val()+' ',
...
Chart.prototype.setTitle = function() {
    ...
    var str = jQuery('#sitename').val()+' ';
    if (this._market_from == "0") {
        // str += '华圣股';
	   str += jQuery('#xnbname').val();
...
Chart.prototype.updateDataAndDisplay = function() {
    ...
    this.setTitle();
    ChartManager.getInstance().setCurrentDataSource('frame0.k0',jQuery('#sitename').val()+'.'+this._market_from+'.'+this._time+'.'+this._money_type+'.'+this._contract_unit);
...
Chart.prototype.setIndicator = function(index, indicName) {
    if (indicName == 'NONE') {
        var areaName = ChartManager.getInstance().getIndicatorAreaName('frame0.k0',2);
        if (areaName != '')
...
ChartManager.prototype.init = function() {
    ...
    DefaultTemplate.loadTemplate("frame0.k0",jQuery('#sitename').val(),"frame0.order","0.order","frame0.trade","0.trade");
...
Range.prototype.preSetRange = function() {};
...
MainRange.prototype.preSetRange = function(r) {
    var mgr = ChartManager.getInstance();
    var timeline = mgr.getTimeline(this.getDataSourceName());
    if (timeline.isLatestShown()) {
        var ds = mgr.getDataSource(this.getDataSourceName());
        var data = ds.getDataAt(timeline.getLastIndex() - 1);
        var d = (r.max - r.min) / 4;
        r.min = Math.min(r.min, data.low - d);
...
DarkTheme.prototype.__construct = function() {
    this._colors = [];
    this._colors[Theme.Color.Positive] = "#1bd357";
    this._colors[Theme.Color.Negative] = "#da1b26";
    ...
    this._colors[Theme.Color.Indicator0] = "#ffffff";
...
MainInfoPlotter.prototype.Draw = function(context) {
    ...
    var title = mgr.getTitle(this.getDataSourceName());
    if (!Plotter.drawString(context, title, rect))
        return;
    if (timeline.getSelectedIndex() < 0)
        return;
    var data = ds.getDataAt(timeline.getSelectedIndex());
...
//                    var admin_dw = array[99][4];
//                    var admin_o = array[99][1];
                    GLOBAL_VAR.KLineData = array;
//                    $("#home_dw", parent.document).html(admin_dw);
//                    $("#admin_dw", parent.document).html(admin_dw);
//                    $("#admin_o", parent.document).html(admin_o);
//                    
...
function setHttpRequestParam(mark_from, time_type, limit, since){
    ...
    str += "&sharesId="+GLOBAL_VAR.sharesId;
    return str;
}
function refreshTemplate() {
    GLOBAL_VAR.chartMgr = DefaultTemplate.loadTemplate(
        "frame0.k0", jQuery('#sitename').val()+GLOBAL_VAR.mark_from+GLOBAL_VAR.time_type,
...
function on_size() {
    ...
    var dropDownSettingsW = dropDownSettings.find(".chart_dropdown_t")[0].offsetWidth + 8;
