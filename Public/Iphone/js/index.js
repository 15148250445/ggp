...
function liststocks(type) {
    ...
    $.get('/Stocks/liststock?type='+type+'&t=' + Math.random(), function(d) {
        ...
        for (var i = 0; i <d.data.length; ++i) {
			if (d.data[i].name == '00008' && current == 'btc') { continue; };
            flag = 'index';
...
function tbtcSubmit(coinname,pid, type){
	//if (coinname == '00008') {
		...
		//return false;
	//	var price =  parseFloat($('#price'+type).val());
	//	if(price < 0.175 || price > 0.7){
	//		$('#trustmsg'+type).css('color','red')
	//						   .html('交易首日限制委托单区间为0.175YBC ~ 0.7YBC')
	//						   .fadeOut(500)
	//						   .fadeIn(500)
	//						   .fadeOut(500)
	//						   .fadeIn(500)
	...
	$.post("/ajax/trustGood/",{type:type,flag:coin.type,pid:pid, coin: coinname, price:parseFloat($('#price'+type).val()), number: parseFloat($('#number'+type).val()), pwdtrade: encodeURIComponent($('#pwdtrade'+type).val()), hotp: encodeURIComponent($('#hotp'+type).val())},
		function(d){
			...
			$('#trustmsg'+type).css('color','#ff6c0b').html(d.msg).fadeOut(500).fadeIn(500).fadeOut(500).fadeIn(500).fadeOut(500).fadeIn(500);
		}, 'json');
}
...
function stocksum(limit){
	...
	$.get('/json/'+coin.name+'_sum?t=' + Math.random(), function(data){
	   ...
	   $('#sale_nice').html(data[buycoin+'_buy'].length==0?data[buycoin+'_sale'][0].p.slice(0,6):data[buycoin+'_buy'][0].p.slice(0,6));
	   $('#buy_nice').html(data[buycoin+'_sale'].length==0?data[buycoin+'_buy'][0].p.slice(0,6):data[buycoin+'_sale'][0].p.slice(0,6));
		...
		if($('#pricein').val() == pricemsg) {
			$('#pricein').val(data[buycoin+'_sale'].length==0?data[buycoin+'_buy'][0].p.slice(0,6):data[buycoin+'_sale'][0].p.slice(0,6));
			buy_max(data[buycoin+'_sale'].length==0?data[buycoin+'_buy'][0].p:data[buycoin+'_sale'][0].p);
		}

		if($('#priceout').val()==pricemsg)
			$('#priceout').val(data[buycoin+'_buy'].length==0?data[buycoin+'_sale'][0].p.slice(0,6):data[buycoin+'_buy'][0].p.slice(0,6));
	}, 'json');
