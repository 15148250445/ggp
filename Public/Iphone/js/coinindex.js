...
function countTime(time){
    ...
    ii--;
    if(ii==-1){
        $("span.retry-send").html('<div style="width: 100px;font-size: 12px;" class="verify phone_code" onclick="sendPhoneCode()">重新发送</div>');
...
function sendPhoneCode(){
	$.post("/ajax/sendmsg2",{type:108,voice:1},function(data){
            ...
            if(data.err == 0){
                ...
                countTime(timecount);
            	$("span.retry-send").html('<div style="width:100px;font-size:12px;background: #ccc;" class="verify"><span class="count-time">'+timecount+'</span>秒后重新发送</div>');
            }
	});
}
	...
	$.post("/ajax/trustcoin/",{type:type, coin: coinname, price:$('#coinprice'+type).val(), number: $('#number'+type).val(), pwdtrade: encodeURIComponent($('#pwdtrade'+type).val()), hotp: encodeURIComponent($('#hotp'+type).val())},
		function(d){
			...
			if(d.status=='passwd_error'){
				$('#pwdtrade'+type).val("");
				Layer.alert(d.msg,function(flag){
					var phone_code=$("#phone_code_trade").val();
					if(flag==1){
						$.ajax({
							type:'post',
							url:'/ajax/tradepwdcheck',
				...
				$('.box_common p').height('60px');
				$('.box_common span').css({'margin-bottom':'10px','font-size':'14px','color':'#666'});
				$('.box_common span.retry-send').css({'display':'block','height':'auto','line-height':'25px'});
				$('.box_common span.showcode').css({'padding-left':'40px','padding-right':'40px','display':'block','height':'auto','line-height':'25px'});
				$('.box_common a.yes.middle').css({'background-color':'#f60'});
...
function currentcoinorder(limit, point, display) {
	...
	$.get('/json/'+coin.name+'_order?t=' + Math.random(), function(d){
		...
		$('#24h_max').html(d.max);
		$('#24h_min').html(d.min);
		if(coin.name=='btc2cny'){
			$('#24h_count').html(formatfloat(d.count*0.5,4,0));
		}else{
